...
> **v0.6 变更**（3 处，都是**实现倒逼出来的**）：
>
> ① **§7.2 第 ③ 组（依赖图校验）补两处**：
>   · 补上「前端与 Java 都要跑」—— 第 ② 组有这一句，第 ③ 组漏了。
>     **这个不对称很可能就是它一直没被实现的原因**：读文档的人看到第 ② 组
>     知道该去哪儿做，看到第 ③ 组只当是个概念说明，就翻过去了。
>   · 补 `E_DANGLING_DEP` 的「为什么需要」：它比同组的成环**更危险** ——
>     因为环至少表现成"整张图卡住了"，而悬空依赖表现成**"没动静"**：
>     那一个任务永远等着，界面上它就是个普普通通的「待办」。
>
> ② **§14.2 新增反模式 #13「静默失败」** —— 本项目已以三种形式
>   （**丢弃** / **跳过** / **替换**）各撞过一次，附三次的实录与落地要求。
>   ⚑ 这条不是从哪条细则推出来的，是本项目**实测的教训**。
>
> ③ **§5.3 新增「删除节点」** —— FR-6 要求"增删"，此前只实现了拖拽，
>   而且**删除的规则代码里有、文档里没有**（DEV_DOC 是权威，这是缺口）。
>   补上两种方式（级联 / 上移一层）和两条必须做的收尾：
>   **清理引用**（不然立刻造出 `E_DANGLING_DEP`）与**同组重编号**。
>
>   同时修正**附录 A 的一处悬空引用**：`E2` 原写「复制时重写引用 → §4.2 复制分支」，
>   而**那一节不存在、本项目也没有"复制"这个需求**（§2.1 的 FR-1～FR-10 里没有）。
>   已改成实际在做的事（结构性改动时重写引用），并把"复制分支"移入「明确不抄」。
>   核查顺带确认附录 A 其余 24 条的引用全部有效。
>
> > 这三处都是"写代码时撞上，回头才发现文档里本来就有、本来就该有、
> > 或者说的根本不是这回事"，与 v0.3–v0.5 那 4 处缺口同一性质。
> > **写代码比读文档更容易发现矛盾。**

> **v0.5 变更**（1 处）：
> §7.3 把 `E_MISSING_EVIDENCE` 限定为「`assignee=agent` 且 done 但无证据」——
> 这条规则是为了防模型的自我报告，而人勾的完成不该被误报。见 §7.3 的说明。
...
| 谁读 | 读哪些 |
|---|---|
| **想快速回看全貌** | → [`ARCHITECTURE.md`](./ARCHITECTURE.md)（导航层：全景图 / 数据流 / 进度 / 约束清单）|
| **我自己（开发者）** | 全文。特别是 §6 ADR、§13 Vibe Coding 边界 |
| **AI 编码助手** | §3 边界铁律、§5 流程、§11 目录结构、§13。**照着这份文档写，不要自由发挥** |
| **面试官** | §1 定位、§6 ADR、附录 B 简历叙事 |

> ⚑ **`ARCHITECTURE.md` 是导航，本文是权威。** 两者冲突时以本文为准 ——
> 导航层若复制了本文的细节，两者必然漂移（§14.2 反模式：镜像双后端，只是发生在文档层）。
...
**两种都合理，所以由用户选** —— 不替人猜（同 §5.3 铁律 #1 的精神用在人工编辑上）。

**两条必须做的收尾。** 不做的话，**用户什么都没干、图上却多出一个错**：

| # | 收尾 | 不做会怎样 |
|---|---|---|
| 1 | **清理引用** —— 把所有 `depends_on` 里指向被删节点的条目删掉 | 立刻造出 `E_DANGLING_DEP`（§7.2 第 ③ 组）：那些任务**永远等着**，而界面上它们只是普普通通的「待办」 |
| 2 | **同组重编号** | 那组留下 `order` 空洞 → `W_ORDER_INVALID` |

> ⚠️ **第 1 条是删节点唯一的真风险。** 它和"跨父移动要重编两组"是同一类问题：
>
> **结构性改动会波及它没有直接碰到的数据**，改动方有责任把波及面收拾干净。
>
> 而这正是 §14.2 #13（静默失败）最容易长出来的地方 ——
> 删除本身干干净净、表面上一切正常，**坏掉的是别人**。
> 「AI 的操作列表里删掉了一个节点，却忘了清理引用它的依赖」
> 就是这个场景的典型写法。

**删除前必须把影响摆给用户看**：会连坐几个节点、以及**有谁的依赖会断**。

> ⚑ 这和上面的「提案 / 应用分离」是同一条道理 ——
> 给 AI 改稿定的规矩是「将新增 3 个节点，删除 1 个」先摆出来再应用。
> **人自己动手删的时候，凭什么不用看后果。**
>
> 而且"删掉一条依赖"就是在**改执行顺序**：用户必须知道有谁从此不再等谁，
> 否则他下次看到那个任务先跑了，会以为 Agent 出错了。

**⚠️ 一种情况不许"上移"**：被删的是**根节点、且它有 2 个以上孩子** ——
上移之后那几个都会变成根，图裂成两棵（`E_MULTIPLE_ROOTS`）。

界面应当**不给这个选项**，而不是给一个灰着的 ——
...
**③ 依赖图**（对**已存 `outline`** 的 `depends_on` —— 前端与 Java 都要跑）

| code | 检查 | 严重度 | 为什么需要 |
|---|---|---|---|
| `E_CYCLE_DEP` | `depends_on` 成环 | 🔴 | 环上的任务互相等对方先做完，谁都不会先跑 |
| `E_DANGLING_DEP` | 依赖了不存在的节点 | 🔴 | 见下 —— **这是全部错误里最难发现的一个** |

> ⚠️ **`E_DANGLING_DEP` 比同组另一个更危险，因为它不表现成"坏了"，而是表现成"没动静"。**
>
> 依赖指向不存在的 id 时，调度器那句
> `depends_on.every(d => byId.get(d)?.status === 'done')` 会求值成 **false** ——
> 于是这个任务**永远等着**，而界面上它就是一个普普通通的「待办」，
> 没有角标、没有报警、控制台干干净净。
>
> 环至少还能从"整张图卡住不动"被察觉；悬空依赖只卡住**那一个**任务，
> 而任务图里本来就有大量任务在等人、等审批，所以"某个任务不动了"根本不显眼。
>
> **静默失灵是这一行里最难查的 bug** —— 宁可多报一条，也不要让它悄悄过去。

> ⚠️ 原文档的 `E_CYCLE` 只覆盖了 `depends_on`，但 `parent_id` 成环是**另一回事**（一个是层级树成环，一个是依赖图成环），必须拆成两个 code —— 它们的检测算法也不同：`parent_id` 是单指针可以"往上走"（O(n²) 足够），`depends_on` 是任意图，**需要真正的图算法**（拓扑排序，或 DFS 三色标记；前端 `lib/outline.ts` 用的是后者，见那里的 `validateDependencies()`）。
>
> ⚠️ 用 DFS 时**必须三色**（白/灰/黑），不能只用"走过 / 没走过"两色 ——
> 两色会把**菱形结构**（两条路通向同一个节点，比如"订机票"和"订酒店"都依赖"决定日期"）
> 误判成环。判断成环的依据只能是那条【从灰指向灰的边】。

> ⚑ **坏数据的处置原则：宁可显式显示异常，也不要静默丢弃。**
> 孤儿节点要渲染成带错误角标的游离节点，不能 `filter` 掉 —— 静默丢弃会让用户看到**内容凭空消失且无从追溯**。
>
> 这是**反模式 #13（静默失败）的一个特例**，见 §14.2 —— 那条是通例，涵盖丢弃 / 跳过 / 替换三种形式。

### 7.3 执行校验 ⭐

| code | 检查 | 严重度 |
|---|---|---|
| `E_MISSING_EVIDENCE` | **`assignee=agent` 且** `done` 但无证据 → **降级为 failed** | 🔴 |
| `E_ASSIGNED_WITHOUT_TOOL` | `assignee=agent` 但无对应工具 | 🔴 |
| `W_OVER_CLAIM` | Agent 声称能做但工具能力存疑 | 🟡 |
| `W_DEP_UNSATISFIED` | 依赖未完成就执行 | 🟡 |

> ⚑ **`E_MISSING_EVIDENCE` 必须限定在 `assignee=agent`**（v0.5 修正）。
>
> 这条规则的存在理由是 §5.2 的证据链规则：「不信任模型的**自我报告**」。
> 而用户自己办完的事**不是**模型的自我报告 —— 由人负责，不需要工具调用记录背书。
>
> 不加这个限定，用户勾掉的每一项都会被误报成"缺少证据链"，
...
| 13 | **静默失败**（回退 / 跳过 / 替换都不出声）| 用户看到的是**错的东西，却以为是对的** | **任何回退都必须留下可见痕迹** |

> ### ⚑ #13 单独说明：这条已经撞到过三次
>
> 三种形式，后果同一个 —— **用户基于错误的信息继续往下做决定。**
>
> | 形式 | 实际发生的那次 |
> |---|---|
> | 静默**丢弃** | 挂不上主树的坏数据被 `filter` 掉 → 内容凭空消失，且无从知道发生过什么（§7.2）|
> | 静默**跳过** | `depends_on` 指向不存在的 id → 那个任务**永远等着**，而界面上它就是个普通「待办」，没角标没报警（§7.2 第 ③ 组）|
> | 静默**替换** | 数据集匹配不上 → 回退到第一份。点「坏数据样本」出来的是日本旅游，什么都没说 |
>
> **为什么它比"报错"更糟：**
>
> 报错会让人停下来、重试、或者至少知道出了问题。
> 静默失败什么都不说，而用户会**照着他以为对的东西继续做**。
> 一个会报错的系统可以被调试；一个静默的系统只能被不信任 ——
> 而"用户可以信任这张图"正是本项目的全部价值（§1.3 的证据链就是为它服务的）。
>
> **落地要求（写代码时可以照着问自己）：**
>
> 任何一处 `continue` / `filter` / 回退 / 默认值，都要能指出一个
> **用户看得见**的出口（issue 条、节点角标、游离节点区……）。
> 指不出来的话，就是这条反模式。
>
> ⚠️ 注意"有出口"不等于"有日志"。日志是给开发看的，用户不会翻 ——
> §1.3 卖点 #3 说得很清楚：用户要**在图上**看见。
...
| | E2 | 稳定 ID + **结构性改动时重写引用** | §4.2 `order` 重编号 · §5.3 删除节点 |
| | E3 | 级联删除（**DB 外键层**）+ DB 拥有时间戳 | §4.1 |
...
| **复制分支**（本表原先的 E2 是这么写的） | 参考项目有、**本项目不需要** —— §2.1 的 FR-1～FR-10 里没有"复制" |

> ### ⚠️ 关于 E2：这里原先有一条**悬空引用**
>
> 原文写的是「稳定 ID + **复制时重写引用**」，指向 **§4.2 复制分支** ——
> 而 **§4.2 根本没有这一节，本项目也没有"复制分支"这个需求。**
>
> 已改成本项目**实际在做**的事：**结构性改动时重写引用**。
> 参考项目那条模式的内核是「ID 稳定、**引用要跟着结构一起改**」，
> 本项目在三处用到它：改层级、改顺序（§4.2 的 `order` 重编号）、删节点（§5.3）。
>
> ⚑ 这类错误比"写错一句话"隐蔽得多：**附录是清单，读的人不会逐条回查。**
> 一条指向空处的条目，会让读者以为那件事已经设计过了 ——
> 而实际上它连需求都不存在。
>
> 本次核查顺带确认了附录 A 其余 24 条的引用**全部有效**
> （§7.4 / §8.4 / §9.1 / §11 下的 `client.ts` `sse.ts` `editor/` `tests/` 都存在）。
